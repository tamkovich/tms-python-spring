...
def Sin1(x: float, n: float) -> float: # Задаем тип параметров вещественные и возврат фунции вещественый
    # создаем слогаемые n > 0
    list_slogaemie  = [(-1)**value * x**(2 * value + 1) / fact(2 * value + 1) for value in range(int(n)) if value > 0]
