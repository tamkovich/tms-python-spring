...
weather_minsk = list(
    filter(lambda row: row if today - seek_period <= datetime.strptime(row[0], '%Y-%m-%d') <= today else None,
           weather_minsk))
...
print(f"The average WIND SPEED for last {seek_period.days} days was {round(avg_wind_speed, 2)} m/s\n"
      f"The average TEMPERATURE for last {seek_period.days} days was {round(avg_temp, 2)} deg")
